      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.statsSummary}>
          <div className={styles.stat}>
            <span className={styles.statLabel}>Total turnos</span>
            <span className={styles.statValue}>{totalTurnos}</span>
          </div>
          <div className={styles.stat}>
            <span className={styles.statLabel}>Total cadetes</span>
            <span className={styles.statValue}>
              {day.stats.totalTurnos || 0}
            </span>
          </div>
        </div>
                  ...
                  <div className={styles.ocupacionBar}>
                    <div
                      className={`${styles.ocupacionFill} ${
                        t.asignados === 0
                          ? styles.vacio
                          : t.asignados >= (day.stats.cupoMax || 4)
                            ? styles.completo
                            : t.asignados > (day.stats.cupoMax || 4) / 2
                              ? styles.medio
                              : styles.bajo
                      }`}
                      style={{
                        width: `${Math.min((t.asignados / (day.stats.cupoMax || 4)) * 100, 100)}%`,
                      }}
                    />
                  </div>
                </div>
